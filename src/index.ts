...
const activate = async () => {
	...
	if (nova.workspace.config.get('taskfinder.auto-node', 'boolean')) {
		...
		const watcher = nova.fs.watch('*package.json', () => nova.workspace.reloadTasks());
	...
	if (nova.workspace.config.get('taskfinder.auto-composer', 'boolean')) {
		...
		const watcher = nova.fs.watch('*composer.json', () => nova.workspace.reloadTasks());
	...
	if (nova.workspace.config.get('taskfinder.auto-taskfile', 'boolean')) {
		...
		const watcher = nova.fs.watch('*Taskfile.*', () => nova.workspace.reloadTasks());
	...
	if (nova.workspace.config.get('taskfinder.auto-maidfile', 'boolean')) {
		...
		const watcher = nova.fs.watch('*maidfile', () => nova.workspace.reloadTasks());
		const extensionWatcher = nova.fs.watch('*maidfile.*', () => nova.workspace.reloadTasks());
